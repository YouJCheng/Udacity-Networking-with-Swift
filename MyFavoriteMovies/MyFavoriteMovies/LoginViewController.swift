    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            func displayError(error:String) {
                ...
                performUIUpdatesOnMain {
                    self.setUIEnabled(true)
                    self.debugTextLabel.text = "Login Fail (Request Token)"
                }
            }
            /* Guard Was there an Error ?*/
            guard error == nil else {
                displayError(error: error.debugDescription)
                return
            }
            /* Did we get Successful 2xx response ?*/
            print(response.debugDescription)
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode,statusCode >= 200 && statusCode <= 299 else {
                displayError(error: "Your Status Code Not in Range")
                return
            }
            ...
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                displayError(error: "TheMovieDB returned an error. See the '\(Constants.TMDBResponseKeys.StatusCode)' and '\(Constants.TMDBResponseKeys.StatusMessage)' in \(parsedResult)")
                return
            }
